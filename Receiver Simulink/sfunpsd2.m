function  [sys, x0, str, ts] = sfunpsd2(t,x,u,flag,fftpts,npts,HowOften,...
                                       offset,ts,averaging,db_range)
...
switch flag
  ...
  %%%%%%%%%%
  case 2
    sys = mdlUpdate(t,x,u,fftpts,npts,HowOften,offset,ts,averaging,db_range);
...
%===========================================================================
%
function [sys]=mdlUpdate(t,x,u,fftpts,npts,HowOften,offset,ts,averaging,db_range)
%
...
if rem(x(1),HowOften)==0
  ...
  while (ng <= (npts - fftpts))
    ...
    g = g + fft(win.*y(ng+1:ng+fftpts),fftpts);
  ...
  ysc2 = [10*log10(ysc(2))-db_range, 10*log10(ysc(2))];
  
  % For the PSD plot.
  set(ud.PSDAxis,...
      'Visible','on',...
      'Xlim',[min(w(2:n)), max(w(2:n))]/(2*pi),...
      'Ylim',ysc2);
  set(ud.PSDInputLine,...
      'XData',w(2:n)/(2*pi),...
      'YData',10*log10(abs(psd)));
  set(ud.PSDTitle,...
      'String', tmp);
  xl = get(ud.PSDAxis, 'Xlabel');
  set(xl,...
      'String','Disc. Frequency')
...
FigHandle = figure('Units',        'points',...
		   ...
		   'DeleteFcn',    'sfunpsd2([],[],[],''DeleteFigure'')');
...
set(ud.TimeHistoryAxis,'Position',[.13 .57 .77 .35]);
...
set(ud.PSDAxis,'Position',[.13 .08 .77 .35]);
